...
import Controller.readCashierReciepts;
import models.*;

import java.util.ArrayList;
import java.util.Scanner;

public class viewMyReciepts implements Option {
    @Override
    public void operation(Scanner s, Database database, User u) {
        ArrayList<Reciept> reciepts= new readCashierReciepts(database,(Cashier) u).getReciepts();
        if(reciepts.size()>0){
