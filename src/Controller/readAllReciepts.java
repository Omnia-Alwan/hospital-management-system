...
public class readAllReciepts {

    private ArrayList<Reciept> reciepts= new ArrayList<>();

    public readAllReciepts(Database database){
        String select="SELECT * FROM `Reciept`";
        try {
            ...
            while (rs.next()){
                ...
                reciept.setPatient((Patient) new ReadPatient(rs.getInt("patient"),database).getPatient());
